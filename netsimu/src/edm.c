                            ...
                            if (pkt->pktType == GRT_TYPE) {
                                ...
                                else if (flow->flowType == RREQ_TYPE) // Received a Grant of a RREQ, means I need to create a new RRESP flow
                                {
                                    if (flow->grantTime < 0) // Check if it is the first time grant
                                    {
                                        ...
                                        rresp_flow->expected_runtime = rresp_flow_size / BLK_SIZE +
                                            2 * per_hop_propagation_delay_in_timeslots;
                                        if (rresp_flow_size > CHUNK_SIZE) {
                                            rresp_flow->expected_runtime += (rresp_flow_size / CHUNK_SIZE + 1) * CHUNK_SIZE / BLK_SIZE +
                                                flow->expected_runtime - rresp_flow_size / BLK_SIZE;// Additional BDP
                                        }
        ...
        if (flowType != NET_TYPE) // is a memflow
        {
            ...
            if (flowType == RREQ_TYPE) {
                ...
                if (rreq_bytes > CHUNK_SIZE) {
                    new_flow->expected_runtime += rreq_bytes / BLK_SIZE + 4 * per_hop_propagation_delay_in_timeslots;
                }
            }
            else {
                new_flow->expected_runtime += min((CHUNK_SIZE * 4), flow_size_bytes) / BLK_SIZE;
            }
            if(flow_size_bytes>CHUNK_SIZE){
                new_flow->expected_runtime += 4 * per_hop_propagation_delay_in_timeslots;
            }
        }
        else {
