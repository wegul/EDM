        ...
        for (int i = 0; i < NUM_OF_NODES; ++i) {
            ...
            if (node->current_flow != NULL) //  we should shut curr whenever cur == waiting
            {
                ...
                // If Curr is not NULL, it must be Granted or NET. So just keep it. But for Net, we have to check quota and see if any mem flow can preempt.
                    ...
                    // Else, if we have a memflow, the current need to be replaced.
            ...
            if (flow) {
                ...
                {
                    int64_t size = flow->flow_size_bytes > CHUNK_SIZE ? CHUNK_SIZE / 4 : BLK_SIZE;
                            ...
                            if (pkt->pktType == GRT_TYPE) {
                                ...
                                else if (flow->flowType == RREQ_TYPE) // Received a Grant of a RREQ, means I need to create a new RRESP flow
                                {
                                    if (flow->grantTime < 0) // Check if it is the first time grant
                                    {
                                        ...
                                        rresp_flow->expected_runtime = rresp_flow->flow_size_bytes / BLK_SIZE + 4 * per_hop_propagation_delay_in_timeslots + 2 * flow->expected_runtime; // Extra 3block of RREQ and its propagation delay
                                        if (rresp_flow->flow_size_bytes > BLK_SIZE * CHUNK_SIZE) {
                                            rresp_flow->expected_runtime += rresp_flow->flow_size_bytes / CHUNK_SIZE + 4 * per_hop_propagation_delay_in_timeslots + 2 * flow->expected_runtime;
                                        }
                                        flow->grantTime = curr_timeslot;
                                        ...
                                        if (rresp_flow_size < CHUNK_SIZE) {
                                            rresp_flow->expected_runtime = (CHUNK_SIZE + 3 * BLK_SIZE) / BLK_SIZE;
                                        }
        ...
        if (terminate0 || terminate1 || terminate2 || terminate3 || terminate4 || terminate5) {
            ...
            for (int i = 0; i < flowlist->num_flows; i++) {
                if (flowlist->flows[i]->finished > 0) {
                    ...
                    if (flowlist->flows[i]->flowType != RREQ_TYPE || flowlist->flows[i]->rreq_bytes < CHUNK_SIZE) {
    ...
    if (flow_id < MAX_FLOW_ID) {
        ...
        if (flowType != NET_TYPE) // is a memflow
        {
            if (flowType == RREQ_TYPE) {
                ...
                flow_size_bytes = burst_size*BLK_SIZE;
            }
            else if (flow_size_bytes < CHUNK_SIZE) {
                flow_size_bytes = CHUNK_SIZE - BLK_SIZE;
            }
            new_flow->grantState = NOTIF_STATE;
            new_flow->expected_runtime = 2 * flow_size_bytes / BLK_SIZE + 4 * per_hop_propagation_delay_in_timeslots; // Simulate zero-notif time
...
int cmp_flow(const void* a, const void* b) {
    ...
    else // tie breaker: shortest remaining first
    {
