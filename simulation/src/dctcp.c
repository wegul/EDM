    ...
    while (1) {
        ...
        for (int i = 0; i < NUM_OF_NODES; ++i) {
            ...
            for (int j = 0; j < node->active_flows->num_elements; j++) {
                ...
                int64_t cwnd_bytes_remaining = node->cwnd[peek_flow0->flow_id] * CHUNK_SIZE - flow_bytes_unacked;
            ...
            if (node->current_flow != NULL) {
                ...
                int64_t cwnd_bytes_remaining = node->cwnd[node->current_flow->flow_id] * CHUNK_SIZE - flow_bytes_unacked;
            ...
            if (flow) {
                ...
                int64_t cwnd_bytes_remaining = node->cwnd[flow_id] * CHUNK_SIZE - flow_bytes_unacked;
                packet_t pkt = NULL;
                ...
                // If no retrans and there are new packets need to be sent (some packets are not received/acked)
                    ...
                    pkt->remain_size = flow->flow_size_bytes - pkt->seq_num - size;
        ...
        for (int tor_port = 0; tor_port < TOR_PORT_COUNT_LOW; ++tor_port) {
            packet_t pkt = NULL;
            pkt = (packet_t)buffer_remove(tor->downstream_send_buffer[tor_port], 0);
            if (pkt) {
            ...
            while (peek_pkt != NULL && peek_pkt->time_to_dequeue_from_link <= curr_timeslot) {
                ...
                if (pkt->control_flag == 0) {
                    ...
                    if (flow->flowType == RREQ_TYPE && pkt->reqLen > 0) {
                        ...
                        rresp_flow->expected_runtime = rresp_flow->flow_size_bytes / BLK_SIZE + 1 + 2 * per_hop_propagation_delay_in_timeslots;// Extra 3block of RREQ and its propagation delay
    ...
    if (flow_id < MAX_FLOW_ID) {
        ...
        if (flowType != NET_TYPE) // is a memflow
        {
            if (flow_size_bytes < CHUNK_SIZE) {
                new_flow->flow_size_bytes += 3 * BLK_SIZE; // 2 cmd + 1 ack
            }
            if (flowType == RREQ_TYPE) {
                new_flow->quota = flow_size_bytes;
                new_flow->flow_size_bytes = 3 * BLK_SIZE;
            }
