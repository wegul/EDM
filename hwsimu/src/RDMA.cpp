    ...
    while (!stop)
    {
        ...
        else
        {
            ...
            if (wqe && wqe->time2deq <= current_time)
            {
                ...
                else // Remote
                {
                    ...
                    if (wqe->flowType == TYPE_RD) // Initial RREQ
                    {
                        wqe->time2deq += PRTCL + RD_BUF + UPD_FST;
                        ctl2net.push(wqe);
                    }
                    else if (wqe->flowType == TYPE_WR) // Initial WREQ
                    {
                        wq.push(wqe);
                    }
                }
        ...
        */
        ...
        if (host_recv_flow && host_recv_flow->time2deq <= current_time)
        {
            ...
            {
                /* Create a response in WQ and fetch DRAM */
                host_recv_flow->flowType = TYPE_RS;
                host_recv_flow->time2deq = current_time + PRTCL + 2 * RD_BUF + UPD_FST; // recv addr, put in grantQ and update FST
